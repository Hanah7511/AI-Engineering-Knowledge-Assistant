...
[bold cyan]╚══════════════════════════════════════════════════════════════╝[/bold cyan]
    ...
    def _initialize_retrievers(self):
        ...
        try:
            ...
            self.retrievers["similarity"] = self.kb.get_retriever(
                k=self.config["retriever_k"],
                search_type="similarity"
            )
            # MMR Retriever
            self.retrievers["mmr"] = self.kb.get_retriever(
                k=self.config["retriever_k"],
                search_type="mmr"
            )
...
💡 PROFESSIONAL ANSWER (with [source] citations):"""
    ...
    def _dynamic_retrieval(self, inputs) -> List[Document]:
        """
        Dynamic retriever wrapper (LCEL FIX)
        Handles both string and dict input from LCEL chain
        """
        # LCEL passes a dict like {"question": "..."}
        if isinstance(inputs, dict):
            question = inputs.get("question", "")
        else:
            question = inputs  # fallback if string
        ...
        try: 
            ...
            try:
                ...
                if hasattr(self.retriever, 'vectorstore'):
                    docs = self.retriever.vectorstore.similarity_search(question, k=self.config["retriever_k"])
                else:
                    docs = self.kb.vector_store.similarity_search(question, k=self.config["retriever_k"]) 
    ...
    def _initialize_chain(self):
        ...
        try:
            ...
            self.chain = (
                RunnableParallel(
                    {
                        "context":  RunnableLambda(self._dynamic_retrieval) |  RunnableLambda(self._format_docs),
    ...
    def ask(
        ...
        try:
            ...
            try:
                print("⚠️ Retriever failed, trying direct similarity search...")
                retrieved_docs = self.kb.vector_store.similarity_search(question, k=self.config["retriever_k"])
                retrieval_time = time.time() - start_retrieval
            except:
                # Restore original retriever
                if retriever_mode:
        ...
        with get_openai_callback() as cb:
            ...
            print("DEBUG: Retrieved docs:", len(retrieved_docs))
    ...
    def calculate_roi(self) -> Dict[str, Any]:
        ...
        minutes_saved_per_query = 15  # Time to manually search docs (in minutes)
        engineer_hourly_rate = 50     # Conservative estimate ($ per hour)

        # Calculate savings
        time_saved_hours = (total_queries * minutes_saved_per_query) / 60
        cost_saved_usd = time_saved_hours * engineer_hourly_rate

        # ROI calculation
        if total_api_cost > 0:
            roi_percentage = ((cost_saved_usd - total_api_cost) / total_api_cost) * 100
        else:
            roi_percentage = float('inf')

        # Per query metrics
        cost_per_query = total_api_cost / total_queries
        value_per_query = cost_saved_usd / total_queries

        # Compile report
        roi_report = {
            "total_queries": total_queries,
            "total_api_cost_usd": round(total_api_cost, 4),
            "total_fallback_queries": total_fallback_queries,
            "avg_confidence": round(avg_confidence * 100, 1),
            "time_saved_hours": round(time_saved_hours, 2),
            "cost_saved_usd": round(cost_saved_usd, 2),
            "net_savings_usd": round(cost_saved_usd - total_api_cost, 2),
            "roi_percentage": round(roi_percentage, 1) if roi_percentage != float('inf') else "∞",
            "cost_per_query_usd": round(cost_per_query, 6),
            "value_per_query_usd": round(value_per_query, 2),
        }
        return roi_report

    def export_data(self, format: str = "all"):
        """
        📤 Export query history to JSON/CSV
        """
        if not self.query_history:
        ...
        # Prepare data
        export_data = [q.to_dict() for q in self.query_history]

        if format in ["json", "all"]:
            json_file = f"exports/query_history_{timestamp}.json"
            with open(json_file, 'w', encoding='utf-8') as f:
                json.dump(export_data, f, indent=2)
            print(f"✅ JSON exported: {json_file}")

        if format in ["csv", "all"]:
            csv_file = f"exports/query_history_{timestamp}.csv"
            with open(csv_file, 'w', newline='', encoding='utf-8') as f:
                import csv
                writer = csv.writer(f)
                # Header
                writer.writerow([
                    "Query", "Timestamp", "Total_Time(s)", "Retrieval_Time(s)",
                    "Generation_Time(s)", "Confidence(%)", "Cost($)", "Sources",
                    "Tokens", "Retriever_Mode", "Fallback_Used", "Cache_Hit"
                ])

                # Data
                for q in self.query_history:
                    writer.writerow([
                        q.query[:100] + ("..." if len(q.query) > 100 else ""),
                        q.timestamp.strftime("%Y-%m-%d %H:%M:%S"),
                        round(q.total_time, 3),
                        round(q.retrieval_time, 3),
                        round(q.generation_time, 3),
                        round(q.confidence_score * 100, 1),
                        round(q.cost_usd, 6),
                        q.num_sources,
                        q.tokens_used,
                        q.retriever_mode,
